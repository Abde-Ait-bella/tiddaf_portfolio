...
(function ($) {
    ...
    if ($(".navbar_nav li.dropdown ul").length) {
        $(".navbar_nav li.dropdown").append(
            '<div class="dropdown-btn"><span class="fa fa-angle-down"></span></div>'
        );
    }
    if ($(".navbar_nav li.mega_menu  ul").length) {
        $(".navbar_nav li.mega_menu").append(
            '<div class="dropdown-btn"><span class="fa fa-angle-down"></span></div>'
        );
    }
    ...
    function vankine_option_panel_popup() {
        if ($(".option_panel-popup").length) {
            ...
            //Hide Popup
            $(".close-option-pan , .option_panel-popup .sdmenu_overlay").on(
                "click",
                function () {
                    $(".option_panel-popup").removeClass("popup-visible");
                }
...
==================================*/
    function vankine_headerStyle() {
        if ($(".sticky_header_content").length) {
            ...
            var sticky_header = $(
                ".sticky_header_content .sticky_header_default"
            );
            if (windowpos > 500) {
                siteHeader.addClass("fixed-header");
                sticky_header.addClass(
                    "wow animate__animated animate__slideInDown"
                );
            } else {
                siteHeader.removeClass("fixed-header");
                sticky_header.removeClass(
                    "wow animate__animated animate__slideInDown"
                );
            }
        }
            ...
            function (e) {
                ...
                $.get($(this).attr("href"), function (response) {
                    var $content = $(response)
                            .find(".portfoliocontainer")
                            .children(".portfolio-wrapper"),
                        protfolio_pagination = $(response)
                            .find(".pagination")
                            .html();
    ...
    function vankine_ser_ajax() {
        $(".ajax_service_enabled").on(
            ...
            function (e) {
                ...
                $.get($(this).attr("href"), function (response) {
                    var $contentser = $(response)
                            .find(".ajaxservicecontainer")
                            .children(".ajax-service-wrapper"),
                        ser_pagination = $(response)
